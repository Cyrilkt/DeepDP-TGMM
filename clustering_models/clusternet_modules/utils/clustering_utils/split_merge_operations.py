...
def split_rule_fixed2(
    ...
    """
    Decide whether cluster k should be split into exactly TWO subclusters.
    Returns: [k, should_split(bool), accepted_local_sub_idx(list)]
    Local subcluster indices are [0,1] if accepted, [] otherwise.
    """
    threshold = 0  # minimum points per (sub)cluster

    # ---- Sanity: exactly two subclusters for cluster k
    n_sub_k = n_sub_list[k] if n_sub_list is not None else 2
    assert n_sub_k == 2, f"Expected exactly 2 subclusters, got {n_sub_k} for cluster {k}"

    # ---- Points in cluster k
    codes_ind = logits.argmax(-1) == k
    codes_k = codes[codes_ind]
    if codes_k.numel() == 0 or len(codes_k) < threshold:
        return [k, False, []]

    # ---- Subcluster offset (flat layout across all clusters)
    subcluster_offset = sum(n_sub_list[:k]) if n_sub_list is not None else (2 * k)

    # ---- Assign to the two subclusters
    if ignore_subclusters:
        # nearest mean among the two provided sub-means
        mus_sub_k = mus_sub[subcluster_offset: subcluster_offset + 2]  # [2, D]
        codes_k_exp = codes_k.unsqueeze(1)     # [Nk, 1, D]
        mus_sub_exp = mus_sub_k.unsqueeze(0)   # [1, 2, D]
        distances = torch.norm(codes_k_exp - mus_sub_exp, dim=2)  # [Nk, 2]
        sub_assignments = distances.argmin(dim=1)                  # [Nk]
    else:
        # argmax over the two logits_sub columns for this cluster
        logits_sub_k = logits_sub[codes_ind, subcluster_offset: subcluster_offset + 2]  # [Nk, 2]
        sub_assignments = logits_sub_k.argmax(-1)  # [Nk]

    # ---- Split codes into the two subclusters
    codes_k_0 = codes_k[sub_assignments == 0]
    codes_k_1 = codes_k[sub_assignments == 1]

    # ---- Threshold / empties guard
    if (len(codes_k_0) <= threshold) or (len(codes_k_1) <= threshold):
        return [k, False, []]

    # ---- Log marginal likelihoods (before & after)
    if prior.prior_choice != 'dynamic_data_std':
        log_ll_k = prior.log_marginal_likelihood(codes_k, covs[k])
        log_ll_0 = prior.log_marginal_likelihood(codes_k_0, covs_sub[subcluster_offset + 0])
        log_ll_1 = prior.log_marginal_likelihood(codes_k_1, covs_sub[subcluster_offset + 1])
    else:
        log_ll_k = prior.log_marginal_likelihood_dynamic_psi(codes_k, covs[k], k)
        log_ll_0 = prior.log_marginal_likelihood_dynamic_psi(codes_k_0, covs_sub[subcluster_offset + 0], k)
        log_ll_1 = prior.log_marginal_likelihood_dynamic_psi(codes_k_1, covs_sub[subcluster_offset + 1], k)

    # ---- Prior-predictive / geometric correction (your q-terms)
    qbefore_split = prior.log_mean_projected_data(codes_k, covs[k])
    qafter_0 = prior.log_mean_projected_data(codes_k_0, covs_sub[subcluster_offset + 0])
    qafter_1 = prior.log_mean_projected_data(codes_k_1, covs_sub[subcluster_offset + 1])
    qafter_split = qafter_0 + qafter_1

    # ---- Sizes
    N0 = int(len(codes_k_0))
    N1 = int(len(codes_k_1))
    # total N_k = N0+N1 by construction

    # ---- Hastings ratio for exactly 2 subclusters (no explicit proposal term)
    should_split = log_Hastings_ratio_split_fixed2(
        alpha=alpha,
        N0=N0, N1=N1,
        log_ll_0=log_ll_0, log_ll_1=log_ll_1, log_ll_k=log_ll_k,
        split_prob=split_prob,
        qbefore_split=qbefore_split, qafter_split=qafter_split,
        device=codes_k.device
    )

    return [k, True, [0, 1]] if should_split else [k, False, []]


def log_Hastings_ratio_split_fixed2(
    alpha, N0, N1, log_ll_0, log_ll_1, log_ll_k, split_prob,
    qbefore_split, qafter_split, device
):
    """
    Computes the log Hastings ratio for a split into EXACTLY TWO non-empty subclusters.
    Acceptance logic matches your original function:
      - Compute H = logJointAfter - logJointBefore
      - If split_prob is None, set split_prob = exp(H)
      - Accept if H > 0 or (split_prob > U) with U ~ Uniform(0,1)
    """
    # tensors on the right device
    alpha_t = torch.as_tensor(alpha, dtype=torch.float32, device=device)
    N0_t = torch.as_tensor(float(N0), dtype=torch.float32, device=device)
    N1_t = torch.as_tensor(float(N1), dtype=torch.float32, device=device)
    Nk_t = N0_t + N1_t

    # Debug prints (optional)
    print("----- Debug Info: Split-2 Components -----")
    print(f"Alpha: {alpha_t.item():.6g}, N0: {N0}, N1: {N1}, Nk: {int(N0+N1)}")
    print(f"log_ll_0: {log_ll_0.item():.6g}, log_ll_1: {log_ll_1.item():.6g}, log_ll_k: {log_ll_k.item():.6g}")
    print(f"qafter: {qafter_split.item() if isinstance(qafter_split, torch.Tensor) else qafter_split}")
    print(f"qbefore: {qbefore_split.item() if isinstance(qbefore_split, torch.Tensor) else qbefore_split}")

    # CRP prior ratio for m=2:
    # (2-1) log alpha + lgamma(N0) + lgamma(N1) - lgamma(Nk)
    crp_term = torch.log(alpha_t) + torch.lgamma(N0_t) + torch.lgamma(N1_t) - torch.lgamma(Nk_t)

    # Data evidence change + q-correction
    like_term = (log_ll_0 + log_ll_1) - log_ll_k
    #q_term = (qafter_split - qbefore_split)
    q_term = qbefore_split

    # Log Hastings (no explicit proposal correction included here)
    H = crp_term + like_term + q_term

    print(f"CRP term: {crp_term.item():.6g}")
    print(f"Like term: {like_term.item():.6g}")
    print(f"q term: {q_term.item() if isinstance(q_term, torch.Tensor) else q_term:.6g}")
    print(f"[Hastings Ratio] H = {H.item():.6g}")

    # Match your acceptance gate
    if split_prob is None:
        split_prob = torch.exp(H).clamp(max=torch.tensor(1.0, device=device))
        print(f"Calculated split probability (exp(H)) = {split_prob.item():.6g}")
    else:
        sp = split_prob.item() if isinstance(split_prob, torch.Tensor) else float(split_prob)
        print(f"User-provided split probability = {sp:.6g}")

    should_split = bool((H > 0) or (split_prob is not None and split_prob > torch.rand(1, device=device)))
...
def update_clusters_params_split_to_del(
    mus,
    covs,
    pi,
    split_decisions,
    ...
    """
    # --- NEW: ensure all tensors live on the same device
    import torch
    device = torch.device("cpu")

    # ensure all inputs live on CPU
    mus     = mus.to(device)
    covs    = covs.to(device)
    pi      = pi.to(device)
    ...
    # Keep clusters not being split, and move them to the correct device
    mus_new  = mus[indices_not_split].to(device)
    covs_new = covs[indices_not_split].to(device)
    pi_new   = pi[indices_not_split].to(device)

    updated_cluster_labels = {}
    updated_prior_labels   = {}

    # -- NEW: { new_cluster_idx: origin_cardinality }
    origin_cardinality = {}
    ...
    for k in split_indices:
        ...
        # pick only accepted subclusters if provided
        ...
        for i in sub_idx_list:
            # move each new tensor to the correct device
            mus_to_add .append(mus_sub_k [i].unsqueeze(0).to(device))
            covs_to_add.append(covs_sub_k[i].unsqueeze(0).to(device))
            pis_to_add .append(pi_sub_k  [i].unsqueeze(0).to(device))
...
def update_clusters_params_split(
    ...
    """
    Update parameters for main clusters after a split.
    Ensures all concatenated tensors live on CPU to avoid device mismatch.
    Returns:
        (mus_new, covs_new, pi_new, prior_new,
         updated_cluster_labels, updated_prior_labels, origin_cardinality)
    """

    # 1) Possibly update prior (unchanged logic, keep original devices for this call)
    if prior.prior_choice == 'dynamic_data_std':
        prior_new = update_clusters_prior_split(
            codes, logits, prior, split_decisions, mus_sub, logits_sub, n_sub_list
        )
    ...
    # ---- Unify devices for indices and params we will concatenate: move to CPU ----
    cpu = torch.device('cpu')

    # split_decisions used for indexing -> keep on CPU
    split_decisions = split_decisions.bool().detach().to(cpu)

    # We'll operate on CPU copies for params involved in concat
    mus_cpu      = mus.detach().to(cpu)
    covs_cpu     = covs.detach().to(cpu)
    pi_cpu       = pi.detach().to(cpu)
    mus_sub_cpu  = mus_sub.detach().to(cpu)
    covs_sub_cpu = covs_sub.detach().to(cpu)
    pi_sub_cpu   = pi_sub.detach().to(cpu)

    # Indices for clusters not split
    indices_not_split = (~split_decisions).nonzero(as_tuple=False).squeeze()
    if indices_not_split.ndim == 0:
        indices_not_split = indices_not_split.unsqueeze(0)

    # Keep clusters not being split (now CPU tensors)
    mus_new  = mus_cpu[indices_not_split]
    covs_new = covs_cpu[indices_not_split]
    pi_new   = pi_cpu[indices_not_split]

    updated_cluster_labels = {}
    updated_prior_labels   = {}

    # NEW: map new_cluster_idx -> number of accepted subclusters that formed it
    origin_cardinality = {}

    new_cluster_idx = 0

    # (A) Not-split clusters: labels + origin_cardinality=1
    for old_idx in indices_not_split.tolist():
        updated_cluster_labels[new_cluster_idx] = cluster_labels[old_idx]
        updated_prior_labels[new_cluster_idx]   = prior_labels[old_idx]
        origin_cardinality[new_cluster_idx]     = 1
    ...
    # For prefix sums over n_sub_list
    # (avoid O(K^2) sums in a loop by accumulating)
    prefix = [0]
    for c in n_sub_list:
    ...
    for k in split_indices:
        ...
        mus_sub_k  = mus_sub_cpu[start_idx:end_idx]
        covs_sub_k = covs_sub_cpu[start_idx:end_idx]
        pi_sub_k   = pi_sub_cpu[start_idx:end_idx]
        ...
        L = len(sub_idx_list)  # number of accepted subclusters

        for i in sub_idx_list:
            mus_to_add.append(mus_sub_k[i].unsqueeze(0))   # CPU
            covs_to_add.append(covs_sub_k[i].unsqueeze(0)) # CPU
            pis_to_add.append(pi_sub_k[i].unsqueeze(0))    # CPU
    ...
    # (C) Concatenate new rows if created (all CPU)
    if mus_to_add:
        mus_to_add  = torch.cat(mus_to_add,  dim=0)
        covs_to_add = torch.cat(covs_to_add, dim=0)
        pis_to_add  = torch.cat(pis_to_add,  dim=0)
    ...
    return (
        ...
        covs_new,         # CPU
        pi_new,           # CPU
        prior_new,        # whatever device prior uses
        updated_cluster_labels,
        updated_prior_labels,
        origin_cardinality
    )

def update_clusters_params_split_LAST_to_decoch(
    mus,
    covs,
    pi,
    split_decisions,
...
def merge_step(
    ...
    covs=None,
    n_merge=2,
    origin_cardinality=None  # dictionary from split step
):
    """
    1) Identify empty clusters => clusters_to_suppress.
    2) Among non-empty clusters, propose merging each with its nearest neighbors
       (number of neighbors governed by origin_cardinality or n_merge).
    3) Apply merge_rule(...) to each proposal, collect accepted merges.
    """
    ...
    for k in range(K):
        cluster_counts[k] = (cluster_assignments == k).sum()
    empty = (cluster_counts == 0).nonzero(as_tuple=False).squeeze().tolist()
    empty_clusters = [empty] if isinstance(empty, int) else empty
    print(f"Empty clusters: {empty_clusters}")

    clusters_to_suppress.extend(empty_clusters)
    clusters_available = set(range(K)) - set(empty_clusters)

    # EARLY EXIT if fewer than 2 clusters remain
    if len(clusters_available) < 2:
        print(f"[merge_step] Only {len(clusters_available)} cluster(s) available => skipping merge.")
        return mus_to_merge, highest_ll_mus, clusters_to_suppress

    print(f"[merge_step] clusters_available={clusters_available}")

    if raise_merge_proposals != "brute_force_NN":
        raise NotImplementedError("[merge_step] Only 'brute_force_NN' is supported.")

    # Prepare data for NN search
    available_indices = sorted(clusters_available)
    mus_available = mus[available_indices]

    # Determine how many neighbors we might ever need
    if origin_cardinality is not None:
        max_card = max(origin_cardinality.get(idx, n_merge) for idx in available_indices)
    else:
        max_card = n_merge

    # CLAMP n_neighbors so that n_neighbors <= n_samples
    global_n_neighbors = min(max(1, max_card - 1), len(available_indices))
    print("GLOBAL N_NEIGHBORS:", global_n_neighbors)

    neigh = NearestNeighbors(n_neighbors=global_n_neighbors, metric="cosine")
    neigh.fit(mus_available.cpu())
    neighbor_indices_all = neigh.kneighbors(return_distance=False)

    # Map row‐indices back to cluster labels
    index_to_cluster = {i: cluster_idx for i, cluster_idx in enumerate(available_indices)}

    visited = set()
    # 2) For each cluster, propose merging with its top neighbors
    for row_idx, cluster_i in enumerate(available_indices):
        if cluster_i in visited:
            continue

        # how many neighbors for THIS cluster?
        if origin_cardinality and cluster_i in origin_cardinality:
            c = origin_cardinality[cluster_i]
        else:
            c = n_merge

        n_neighbors_i = max(1, min(c - 1, len(available_indices) - 1))
        print("CLUSTER_i:", cluster_i, "→ nb_neighbors_i:", n_neighbors_i)

        nbr_rows = neighbor_indices_all[row_idx][:n_neighbors_i]
        neighbor_clusters = [index_to_cluster[r] for r in nbr_rows]

        proposed = list({cluster_i, *neighbor_clusters})
        # filter out any suppressed clusters
        proposed = [p for p in proposed if p in clusters_available]

        if len(proposed) < 2:
            visited.update(proposed)
            continue

        print(f"[merge_step] Proposing to merge group={proposed}")
        decision, ll = merge_rule(
            mus=mus,
            logits=logits,
            codes=codes,
            cluster_indices=proposed,
            alpha=alpha,
            cov_const=cov_const,
            merge_prob=merge_prob,
            prior=prior,
        ...
        print(f"[merge_step] Decision={decision}, ll={ll}")
        if decision[0]:
            # accept: remove them from future consideration
            for p in proposed:
                clusters_available.discard(p)
            mus_to_merge.append(proposed)
            highest_ll_mus.append(ll)
            print(f"[merge_step] Merged group: {proposed}")
        else:
            print(f"[merge_step] Merge refused for {proposed}.")
...
def log_Hastings_ratio_merge_vdeterministic(alpha,
    N_s_list,
    log_ll_s_list,
    N_c,
    log_ll_merged,
